...
class SignListener : Listener {
    ...
    companion object {
        ...
        private fun applyColorsOnSign(event: SignChangeEvent, player: Player) {
            val colorCharacter =
                SignColors.instance.config.getString(ConfigKey.COLOR_CHARACTER.toString())
        ...
        private fun checkColorPermissions(player: Player, color: Int): Boolean {
            ...
            return (color == 0) ||
                player.hasPermission(Permissions.USE_SPECIFIC_COLOR.toString() + colorChar) ||
        ...
        private fun applyLinesOnSpecialSign(event: SignChangeEvent, amount: Int, price: Double) {
            with(event) {
                ...
                setLine(
                    ...
                    SignColors.languageConfig.get(LanguageKey.SPECIAL_SIGN_LINE_TWO)!!.color()
                )
                setLine(SIGN_LINE_THIRD_INDEX, "&8$amount : $price".color())
                setLine(
                    SIGN_LINE_LAST_INDEX,
                    SignColors.languageConfig.get(LanguageKey.SPECIAL_SIGN_LINE_FOUR)!!.color()
                )
            }
        }
    }
                ...
                if (dataLine!!.isNotEmpty() && dataLine.contains(":")) {
                    ...
                    with(SignColors.instance.config) {
                        applyLinesOnSpecialSign(
                            event,
                            getInt(ConfigKey.SIGN_AMOUNT_SPECIAL_SIGN.toString()),
    ...
    fun checkFirstLineOnSpecialSignCreation(event: SignChangeEvent) {
        ...
        if (
            firstLine == SPECIAL_SIGN_INDICATOR &&
                player.hasPermission(Permissions.SPECIAL_SIGN_CREATE.toString())
        ) {
            return
        }
        ...
        val blockedLines =
            SignColors.instance.config.getList("blocked_first_lines")!!.filterIsInstance<String>()
