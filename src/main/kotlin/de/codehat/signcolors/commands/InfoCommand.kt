...
class InfoCommand: Command() {
    ...
    override fun onCommand(sender: CommandSender,
        ...
        with (sender) {
            sendColoredMsg("&6+--------------------&r${SignColors.languageConfig
					.get(LanguageKey.TAG)}&r&6--------------------+")
            sendColoredMsg(" ${SignColors.languageConfig.get(LanguageKey.INFO_AUTHOR)} &a${SignColors
					.instance.description.authors.joinToString(",")}")
            sendColoredMsg(" ${SignColors.languageConfig.get(LanguageKey.INFO_VERSION)} &a${SignColors
